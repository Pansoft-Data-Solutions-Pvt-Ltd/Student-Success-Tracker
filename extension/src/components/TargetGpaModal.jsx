...
const TargetGpaModal = ({
  ...
  // ── Ref for the result section to capture as PDF ──
  ...
  const hasError = Boolean(
    targetGpaInput &&
    ((() => {
      const n = parseFloat(targetGpaInput);
      return isNaN(n) || n < 0 || n > (parseFloat(maxGpa) || 4.0);
    })() ||
  ...
  return (
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth="md">
      ...
      >
        ...
        {!result && !loading && (
          <>
            <Typography
              variant="body2"
              style={{ color: "#210c50", marginBottom: "14px" }}
            >
            ...
            {showBelowGpaBanner && (
              ...
              >
                ...
                <div>
                  ...
                  >
                    Your target GPA (
                    <strong>{parseFloat(targetGpaInput).toFixed(2)}</strong>)
                    must be <strong>greater than</strong> your current Program
                    GPA of <strong>{parseFloat(programGpa).toFixed(2)}</strong>.
        ...
        {/* ── Result ── */}
        {parsed && !loading && (
          // ↓ ref attached here — only this section is captured in the PDF
          <div
            ref={resultRef}
      ...
      >
        {/* "Get AI Recommendation" button — shown only before result */}
        {!result && (
          <button
            ...
            style={{
              textTransform: "none",
              backgroundColor:
                loading || !targetGpaInput || hasError ? "#d8b4fe" : "#300e4d",
              color: "#ffffff",
              border: "none",
              borderRadius: "4px",
              padding: "8px 16px",
              cursor:
                loading || !targetGpaInput || hasError
                  ? "not-allowed"
                  : "pointer",
              letterSpacing: "0.3px",
            }}
          >
            <Typography
              variant="body2"
              style={{ color: "#ffffff", fontWeight: 600, margin: 0 }}
            >
        ...
        {/* "Download PDF" button — shown only when result is available */}
        {result && (
          <button
            onClick={downloadPdf}
            disabled={pdfLoading}
